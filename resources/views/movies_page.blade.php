          ...
          <div class="card  my-3 border-0">
              <div class="row mb-3">
                ...
                @foreach ($slug as $slugslink)
                  @if ($mv['id'] == $slugslink->id_movies)    
                  <div class="col-sm-2 col-6 text-center">
                    <div class="card border-0">
                      <div class="img-overflay mb-3"> 
                        <a href="{{ route('film.show', $slugslink->slug) }}">   
                      <img src="https://image.tmdb.org/t/p/w500/{{ $mv['poster_path'] }}" class="card-img-top" alt="...">
                    </a>
                      <span class="btn btn-warning label-image-top">Rate:{{ $mv['vote_average'] }}</span>
                  </div>
                  <h5>{{ $mv['title'] }}</h5>
                  <p>{{ Str::substr($mv['overview'], 0, 60)."..." }}</p>
                  <a href="{{ route('film.show', $slugslink->slug) }}">
                  <button class="btn btn-dark text-center mb-3">Nonton Film</button>
                  </a>
                    </div>
                </div>   
                  @else
...
<div class="d-flex justify-content-center">
  {{ $movies->onEachSide(1)->links('pagination::bootstrap-4') }} 
</div>
@endsection